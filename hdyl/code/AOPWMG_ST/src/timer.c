...
uint32_t timecount,test,timecount1,fq=-1;
...
void TIM3CaptureChannel3Callback()//捕获中断发生时执行
{
  ...
  #else
		if(SET == timer_interrupt_flag_get(TIMER3,TIMER_INT_CH3)){
			   /* clear channel 0 interrupt bit */
			   timer_interrupt_flag_clear(TIMER3,TIMER_INT_CH3);
		
			   if(0 == state){
				   /* read channel 0 capture value */
				   timecount = timer_channel_capture_value_register_read(TIMER3,TIMER_INT_CH3)+1;
				   state = 1;
			   }else if(1 == state){
				   /* read channel 0 capture value */
				   timecount1 = timer_channel_capture_value_register_read(TIMER3,TIMER_INT_CH3)+1;
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	...
	#else
	if(SET == timer_interrupt_flag_get(TIM3,TIMER_INT_CH3)){
	TIM3CaptureChannel3Callback();
		}
