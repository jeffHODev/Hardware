...
(
    ...
    number         int          not null comment '购买使用的总次数',
    state          tinyint      not null comment '购买状态（0 已完成 1 待支付）',
    hospital_id    varchar(64)  not null comment '购买医院',
    device_no      varchar(20)  not null comment '购买的设备号',
    del_flag       tinyint      not null comment '是否删除（0正常 1删除）',
    PRIMARY KEY (`id`)
);
