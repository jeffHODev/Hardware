package com.hekangyuan.nivi1000.view.bargraph;
...
public class BarGraphView extends View {
    ...
    public BarGraphView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        //柱状图的颜色
        barGraphBgColor = typedArray.getColor(R.styleable.BarGraphView_bar_graph_bg_color, Color.BLACK);
        //X轴字体大小
    ...
    private void measureWidth(int heightMeasureSpec) {
        ...
        //左边字体的长度
        widthMeasureSpec += mLeftYWidth + dip2px(10)+500;
    ...
    private void drawXYText(Canvas canvas) {
        if (isShowYText) {
            //Y轴写字
            for (int i = 1; i <= 6; i++) {
                float startY = bottomHeight - bottomHeight * 0.9f / maxHeight * maxHeight / 6 * i;
                canvas.drawLine(dip2px(10) + mLeftYWidth, startY, dip2px(15) + mLeftYWidth, startY, mYTextPaint);
                float width = mYTextPaint.measureText(maxHeight / 6 * i + "");
                Paint.FontMetricsInt fontMetricsInt = mYTextPaint.getFontMetricsInt();
                float dy = (fontMetricsInt.bottom - fontMetricsInt.top) / 2 - fontMetricsInt.bottom;
                canvas.drawText(maxHeight / 6 * i + "", (int) (dip2px(10) + mLeftYWidth - width - dip2px(6)), startY + dy, mYTextPaint);
            }
        }
        if (!isShowXText) {
            return;
        ...
        if (barGraphTextList != null && barGraphTextList.length > 0) {
            for (int i = 0; i < barGraphTextList.length; i++) {
                ...
                canvas.drawText(barGraphTextList[i], startX - textWidth / 2+250, heightMeasureSpec - dip2px(5), mXTextPaint);
            }
        }
    }
    ...
    private void drawBarGraph(Canvas canvas) {
        if (barGraphDataList != null && barGraphDataList.length > 0) {
            for (int i = 0; i < barGraphDataList[0].length; i++) {
                float startX = mLineSpaceWidth * (i + 1) + mLineWidth * barGraphDataList.length * i + mLeftYWidth + dip2px(10) + mLineWidth / 2+250;
                int index = 0;
