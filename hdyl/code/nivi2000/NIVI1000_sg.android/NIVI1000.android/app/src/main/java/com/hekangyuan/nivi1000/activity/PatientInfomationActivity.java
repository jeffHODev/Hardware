...
//        });
        et_birthday.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            ...
            public void onFocusChange(View view, boolean b) {
                if(b){
                        if(et_uniqueId.getText().toString().trim().length() == 0 || "0".equals(et_uniqueId.getText().toString().trim())){
                            showToast("请先填写ID");
...
//                            showToast("ID格式不正确");
                            ...
                            if(et_birthday.getText().toString().equals("")){
                                et_birthday.setText(CheckIdCard.getBirthday1(et_uniqueId.getText().toString()));
//                                et_birthday.clearFocus();
                            }
                        }
                    }
                    ...
                    if(level1 == 1){
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if(patientInformationBean.getUniqueId().trim().length() == 0 || "0".equals(patientInformationBean.getUniqueId().trim())){
                                    showToast("请填写ID");
                                    return;
                                }
...
//                                }
                                ...
                                if(patientInformationBean.getBirthday().length() != 8){
                                    showToast("请输入8位数的出生日期");
                                    return;
                                }
                                if(!CheckIdCard.chkDateFormat(patientInformationBean.getBirthday().substring(0,4) + "-" + patientInformationBean.getBirthday().substring(4,6) + "-" + patientInformationBean.getBirthday().substring(6,8))){
                                    showToast("出生日期不合法");
                                    return;
                                }
                                if(sex == null){
...
//            et_birthday.setText(patientInformationBean.getBirthday());
            et_birthday.setText(patientInformationBean.getBirthday().substring(0,4) + patientInformationBean.getBirthday().substring(5,7) + patientInformationBean.getBirthday().substring(8,10));
