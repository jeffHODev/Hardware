...
uint16_t EE_Init(void)
{
  ...
  uint32_t  FlashStatus;
	...
	FLASH_EraseInitTypeDef FLASH_Erase_Type;
  ...
  switch (PageStatus0)
  {
    case ERASED:
      ...
      {
        /* Erase Page0 */
				FLASH_Erase_Type.TypeErase=FLASH_TYPEERASE_PAGES;
				FLASH_Erase_Type.PageAddress=PAGE0_BASE_ADDRESS;
				FLASH_Erase_Type.NbPages=1;
				HAL_FLASHEx_Erase(&FLASH_Erase_Type, &FlashStatus);
        /* If erase operation was failed, a Flash error code is returned */
        if (FlashStatus != 0xFFFFFFFF)
        {
          return FlashStatus;
        }
      }
    ...
    case RECEIVE_DATA:
      if (PageStatus1 == VALID_PAGE) /* Page0 receive, Page1 valid */
      {
        ...
        /* Erase Page1 */
				FLASH_Erase_Type.TypeErase=FLASH_TYPEERASE_PAGES;
				FLASH_Erase_Type.PageAddress=PAGE1_BASE_ADDRESS;
				FLASH_Erase_Type.NbPages=1;
				HAL_FLASHEx_Erase(&FLASH_Erase_Type, &FlashStatus);				
        /* If erase operation was failed, a Flash error code is returned */
        if (FlashStatus != 0xFFFFFFFF)
...
static uint32_t EE_Format(void)
{
  uint32_t FlashStatus = 0xFFFFFFFF;			// (0xFFFFFFFF means that all the pages have been correctly erased)
	FLASH_EraseInitTypeDef FLASH_Erase_Type;
  /* Erase Page0 */
	FLASH_Erase_Type.TypeErase=FLASH_TYPEERASE_PAGES;
	FLASH_Erase_Type.PageAddress=PAGE0_BASE_ADDRESS;
...
static uint32_t EE_PageTransfer(uint16_t VirtAddress, uint16_t Data)
{
  ...
  /* Erase the old Page: Set old Page status to ERASED status */
	FLASH_Erase_Type.TypeErase=FLASH_TYPEERASE_PAGES;
	FLASH_Erase_Type.PageAddress=OldPageAddress;
	FLASH_Erase_Type.NbPages=1;
	HAL_FLASHEx_Erase(&FLASH_Erase_Type, &FlashStatus);
  /* If erase operation was failed, a Flash error code is returned */
  if (FlashStatus != 0xFFFFFFFF)
