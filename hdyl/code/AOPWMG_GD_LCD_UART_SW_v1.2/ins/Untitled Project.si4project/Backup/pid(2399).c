...
//	pp->LastError=Error;
    ...
    if(NextPoint>(pp->SetPoint+100))    //如果上一次输出变量出现正向的饱和
    {
        if(abs(Error)>200)
        {
            index = 0;//偏差大积分不起作用
    ...
    else if(NextPoint<(pp->SetPoint-100))  //如果上一次输出变量出现负向的饱和
    {
        if(abs(Error)>200)
        {
            index = 0;//偏差大积分不起作用
    ...
    {
        if(abs(Error)>200)     //积分分离的PID优化，可参考以往的文章
        {
            index = 0;//偏差大积分不起作用
        }
        else
