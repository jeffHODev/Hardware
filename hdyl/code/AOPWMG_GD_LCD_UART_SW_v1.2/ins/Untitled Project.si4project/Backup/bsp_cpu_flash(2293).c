...
addr = _ulFlashAddr + FLASH_BASE_ADDR;
    ...
    fmc_read_32bit_data(_ulFlashAddr+i+FLASH_BASE_ADDR, _ulSize, _ucpDst);
	#endif
...
//	uint16_t usTemp;
   ...
   /* write 8 bit length data to a given address */
	...
	fmc_write_32bit_data(_ulFlashAddr+i+FLASH_BASE_ADDR, _ulSize,_ucpSrc);
...
void FlashEease(uint32_t _ulFlashAddr)
{
    ...
    fmc_erase_sector_by_address(_ulFlashAddr);	
	#endif
