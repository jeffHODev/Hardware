...
void GetTds_EleCurr()
{
    ...
    float tds_tmp;
    registerTick(ADC_TICK,70,1,0);//70ms采集一次
    ...
    if(GetTickResult(ADC_TICK)==1)
    {
        ...
        if(init == 0)
        {
            averTdsO1 = adc1_val_buf[5];//原水tds
            averTds =  adc1_val_buf[1];//盐水tds
...
void GetFlow()
{
    ...
    {
		flow_cnt++;
        sensor.sum_ele_time = flow_cnt/1200/60.0;
	}
    ...
    if(*GetCapture() != -1)
    {
        ...
        if(init_flag == 0)
        {
			...
			sensor.water_quantity = flow_cal/FLOW_RATIO;
