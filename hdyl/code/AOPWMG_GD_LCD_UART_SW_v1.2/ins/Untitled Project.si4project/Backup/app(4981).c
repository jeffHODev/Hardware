  ...
  //// if(GetSensor()->flow>0&&(GetSensor()->status[WASH_INDEX]||GetSensor()->status[NORMAL_INDEX] == 20))
   	{
		...
		DcMotorCtrl(1, 65535);
		if((HAL_GetTick()-params_tick)>=6000)
		{
			SystemParamsSave();
...
void SystemParamsRead()
{
   uint32_t addr,len;
    ...
    {
			addr = addr +4;
            bsp_ReadCpuFlash(addr,(uint32_t *)(&GetSensor()->water_quantity), 1);
			addr = addr +4;
			bsp_ReadCpuFlash(addr,(uint32_t *)(&GetSensor()->sum_ele_time), 1);
