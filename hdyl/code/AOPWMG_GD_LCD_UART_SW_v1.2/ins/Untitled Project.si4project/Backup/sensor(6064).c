...
//*************************************************************************************
        ...
        printf("		 %d 	   %d	   %d\n",adc1_val_buf[0],adc1_val_buf[1],adc1_val_buf[5]);
        init = 1;
...
void GetFlow()
{
    ...
    if(*GetCapture() != -1)
    {
        ...
        else
        {
            ...
            sensor.flow  = (flow_tmp-16)/24.0f;//20 18 
            *GetCapture() = -1;
        }
    ...
    if( sensor.flow>FLOW_THRES)
        ;//	sensor.flow = sensor.flow-FLOW_THRES;
...
unsigned char GetPH_ORP()
{
    ...
    registerTick(SENSOR_TICK_NO,450,1,0);
            ...
            if(GetSensor()->wash_time<=MAX_WASH_TIME)
            {
...

                {
                    ...
                    else
                    {
                        ...
                        GetSensor()->status[ORP_INDEX]||GetSensor()->status[PH_INDEX]||GetSensor()->status[WASH_INDEX])//wash invert ele
                {
                    tx_buf[0] = 0;
                    tx_buf[1] = 1;
                    if(addr_tmp<=M4_ADDR)
...
void orp_ph_process(void)
{
	...
	while(getModbusPayload()->RS485_FrameFlag !=2&&result ==1)
	{
		 registerTick(MODBUS_TIMEOUT_TICK_NO,50,1,0);
	    if(GetTickResult(MODBUS_TIMEOUT_TICK_NO)==1)
	    {
	        registerTick(MODBUS_TIMEOUT_TICK_NO,0,0,1);
			goto next;
		}
	}
	next:registerTick(MODBUS_TIMEOUT_TICK_NO,0,0,1);
