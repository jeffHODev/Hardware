...
void hsw_proc()
{
    ...
    registerTick(FLOW_TICK_NO2, 8000,1,0);//
...
void normal_proc()
{
    ...
    if(GetSensor()->status[SHUNT_INDEX] == 0)//非停机模式
    {
        ...
        else//水位正常，其他参数检测才有意义
        {
            ...
            if((GetSensor()->flow-FLOW_SIZE)>=0.5||((GetSensor()->flow-FLOW_SIZE)<=-0.5))	//水流量不足
            {
                flow_low_cnt++;
    ...
    for(i=1; i<WORK_STATUS_SIZE; i++)
    {
        ...
        if(GetSensor()->status[i]!=0)
        {
            ...
            if(i!=ORP_INDEX&&i!=PH_INDEX)
            {
                ...
                if(i!=WATER_LEVEL_INDEX&&i!=WASH_INDEX)
                    beep_pro(6,0);
                break;
            }
