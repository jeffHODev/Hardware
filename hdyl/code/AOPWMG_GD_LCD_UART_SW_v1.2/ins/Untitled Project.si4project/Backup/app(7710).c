...
uint32_t dstTds=1000;
unsigned char module_reset_flag=0;
work_params_stru work_params;
static unsigned char shunt_flag=0;

unsigned char	*str1 = "状态正常";
unsigned char	*str2 = "原水TDS异常";
unsigned char	*str3 = "进水TDS异常";
unsigned char	*str4 = "流量异常";
unsigned char	*str5 = "水压异常";
unsigned char	*str6 = "水位异常";
unsigned char	*str7 = "电解工作中";
unsigned char	*str8 = "清洗中";
unsigned char	*str9 = "待机中";
unsigned char	*str10 = "参数异常";
...
unsigned char abnormalDec()
{
    ...
    if(GetSensor()->flow >= MAX_FLOW|| GetSensor()->flow <= MIN_FLOW)//流量异常
    {
        ...
        {
            status = status | TickTimeoutAb(FLOW_TICK_NO,0x01,10000);
        }
    ...
    {
        status = status & TickTimeoutNor(FLOW_TICK_NO,0xfe,2000);
    }
...
//********************************************************************************
    if(GetSensor()->tds1>= MAX_TDS1_VALUE|| GetSensor()->tds1 <= MIN_TDS1_VALUE)//原水tds异常
    {
        ...
        status = status | TickTimeoutAb(TDS1_TICK_NO,0x40,MAX_TICK);
...
polling_status:
	...
	#else
	    if(status_tmp==20)
		SetLableValueColor(Main_PAGE,LABEL_WORK_ID, BACK_COLOR,FORCE_COLOR, str);
		else
		SetLableValueColor(Main_PAGE,LABEL_WORK_ID, WARN_BACK_COLOR,FORCE_COLOR, str);			
	#endif
        //
    //free(str2);
...
module_reset(unsigned char mode)
{
    ...
    registerTick(MODULE_RESET_TICK, 500, 1,0);//
...
void dev_wash_ctrl()
{
    GetSensor()->wash_time =GetSensor()->wash_time +1 ;//大于3次停机
    GetSensor()->err_flag = 0;
...
void hsw_proc()
{
    ...
    registerTick(FLOW_TICK_NO2, 2000,1,0);//
    ...
    if(GetSensor()->status[SHUNT_INDEX] == 0)//非停机模式
    {
        ...
        else//水位正常，其他参数检测才有意义
        {
            ...
            else  //流量正常
            {
                ...
                GetSensor()->status[NOWATER_INDEX] = 0;//缺水
                if((abnormalDec()&0x33)==0)//无异常，走正常逻辑 0x73 0x33
                {
                ...
                else //参数有异常进行调节控制
                {
                    ...
                    {
                        if(GetSensor()->err_flag&0x10)//tds报警,自动清洗
                        {
                            registerTick(WASH_TICK, WASH_TIME_SETTING,1, 0);
                            if( GetTickResult(WASH_TICK)==1)//清洗时间到
                            {
                                dev_wash_ctrl();
                                ...
                                else
                                {
                                    if(GetSensor()->wash_time<3)
                                    {
                                        if(GetSensor()->status[HSW_INDEX]==0)
                                            GetSensor()->status[TDS2_INDEX] = 2;//tds2异常
                                        EleSwCtrl(WATER_SW,ON);//原水进水阀开
                                        EleSwCtrl(WASTE_SW,ON);//废水出水阀开
...
void ele_dev_proc()
{
    ...
    if(GetSensor()->wash_time>=3&&shunt_flag ==0||GetSensor()->status[TDS1_INDEX]==1)
    {
        shunt_flag = 1;
...
void ele_process()
{
    ...
    else if(GetInOut()->key_cali_mode != 0)
    {
        GetInOut()->key_wash_mode = 0;
        // registerTick(WASH_TICK, 0,0,1);
