...
void Flow_Init()
{
    ...
    else
    {
        ...
        if(GetTickResult(TDS_SETTING_TICK_NO)==1)
        {
            ...
            else if(GetSensor()->flow<=3.5)//3---3.5 500-650
            {
            ...
            else if(GetSensor()->flow<=4)//3.5---4    650-950	+30----0.1
            {
                flow_switch = (GetSensor()->flow-3.5)*600+650;
                //if(flow_switch<TDS_LEVEL3)
                 //   flow_switch = TDS_LEVEL3;
            }
            else
...
void ele_process()
{
    ...
    if(GetInOut()->key_wash_mode)//冲洗状态
    {
        pump_ctrl(0);
        if(abnormalDec()&0x20)//水位异常
        {
...
void SystemParamsSave()
{
	...
	tmp = GetSensor()->sum_ele_time*10;
	GetSensor()->sum_ele_time = tmp/10.0;
