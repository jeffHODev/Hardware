...
void GetFlow()
{
    ...
    if(*GetCapture() != -1)
    {
        ...
        if(init_flag == 0) 
        {
            ...
            init_flag = 1;
            sensor.flow  = flow_cal*1200.0;//1200;
					  sensor.flow  = (sensor.flow/FLOW_RATIO);//1200;
        }
    ...
    else//flow is zero
    {
        ...
        if((HAL_GetTick()-flow_tick)>=3000)
        {
           ...
           // if(flow_cal <= 0)
            ...
            //{
			//	if(flow_cnt>0)
			//		sensor.flow  = (flow_tmp*2.4)/(flow_cnt);//1200;
			//	else
				//	sensor.flow  = 0;
			//} 
