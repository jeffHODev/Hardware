...
void lcd_touch_interrupt(void)
{
    ...
    {
        command_cmd = IRcvByte(SLA_WR);
        queue_push(command_cmd);//压入到指令缓冲区
        for(cnt =0; cnt <2; cnt++)
        {
            identifier = IRcvByte(SLA_WR);
            queue_push(identifier);//压入到指令缓冲区

        }
        command_statu = IRcvByte(SLA_WR);

        queue_push(command_statu);
        identifier = IRcvByte(SLA_WR);

        queue_push(identifier);
        command_length = IRcvByte(SLA_WR);
        ...
        getTouch()->command = getQueue()->_data[0];
				getTouch()->control_id = getQueue()->_data[1];
		if( getTouch()->control_id == PH1_CAL_ID)
		{
			//if(getQueue()->_data[1]==PH1_CAL_OK_ID)
				getTouch()->control_id = getQueue()->_data[1];
			    getTouch()->last_ctrl_id = getQueue()->_data[1];
		}
		else if( getTouch()->control_id == PH2_CAL_ID)
		{
			//if(getQueue()->_data[1]==PH2_CAL_OK_ID)
				getTouch()->control_id = getQueue()->_data[1];
			    getTouch()->last_ctrl_id = getQueue()->_data[1];
		...
		{
			//if(getQueue()->_data[1]==ORP_CAL_OK_ID)
			getTouch()->control_id = getQueue()->_data[1];
			getTouch()->last_ctrl_id = getQueue()->_data[1];
		...
		getTouch()->page_id = getQueue()->_data[2];			
		getTouch()->status = getQueue()->_data[3];
