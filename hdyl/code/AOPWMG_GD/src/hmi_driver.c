...
static uint8_t len;
#define TX_8(P1) ADD_DATA((P1)&0xFF)  //发送单个字�?
#define TX_8N(P,N) ADDNU8((uint8_t *)P,N)  //发送N个字�?
#define TX_16(P1) TX_8((P1)>>8);TX_8(P1)  //发�?6位整�?
...
void ADDNU8(uint8_t *pData,uint16_t nDataLen)
{
	uint16_t i = 0;
	for (;i<nDataLen;++i)
	{
