...
uint8_t bsp_WriteCpuFlash(uint32_t _ulFlashAddr, uint8_t *_ucpSrc, uint32_t _ulSize)
{
	...
	for(i = 0; i<_ulSize;i++)
	{
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD,_ulFlashAddr+i, *(uint64_t *)(_ucpSrc+i));
	...
	for(i = 0; i<_ulSize;i++)
	{
	 fmc_program( _ulFlashAddr , _ucpSrc[i]);
...
void FlashEease(uint32_t _ulFlashAddr)
{
	#if CPU == ST
	HAL_FLASH_Unlock();
	FLASH_PageErase(bsp_GetSector(_ulFlashAddr));
	HAL_FLASH_Lock();
	#else

	fmc_erase_pages(_ulFlashAddr);
	#endif
}
