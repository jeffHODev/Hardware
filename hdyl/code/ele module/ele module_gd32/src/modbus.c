...
//	   dma_parameter_struct dma_init_struct;
    modbus_usr.RS485_FrameFlag=0;//置位帧结束标记
#if CPU == ST
    HAL_UART_DMAStop(&huart1);
    HAL_UART_DMAResume(&huart1);
    HAL_UART_Receive_DMA(&huart1,modbus_usr.RS485_RX_BUFF,MODBUS_BSIZE);
	...
	if(modbus_usr.RS485_RX_BUFF_tmp[0] == modbus_usr.RS485_Addr&&(modbus_usr.RS485_RX_BUFF_tmp[1]==0x03||
		modbus_usr.RS485_RX_BUFF_tmp[1]==0x06))
		{
			 memcpy(modbus_usr.RS485_RX_BUFF,modbus_usr.RS485_RX_BUFF_tmp,8);
...
void buffer_reset()
{
  ;//memset( modbus_usr.RS485_RX_BUFF,0,MODBUS_BSIZE);
}
